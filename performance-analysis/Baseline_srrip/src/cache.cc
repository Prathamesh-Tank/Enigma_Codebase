...
//Added by prath for MIRAGE_extention

//int temp_valid_ways = NUM_WAY;
//bool access_DA[LLC_SET] = {false};
//int valid_blocks[LLC_SET] = {LLC_WAY};
...
void CACHE::handle_fill() //Interconnect done
{
	...
	if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
		...
		if(MIRAGE == 1 && cache_type == IS_LLC)
		{
			...
			if(MSHR.entry[mshr_index].sos_status == true)
			{
				set = MSHR.entry[mshr_index].ssl;      //prath_set();//MSHR.entry[mshr_index].da_set;
				//cout<<"set: "<< set <<endl;
				way = MSHR.entry[mshr_index].sos_way;   //prath_way(set);
				if(tag_number ==0 )
			...
			else
			{
				...
				set = MSHR.entry[mshr_index].ssl;      //prath_set();//MSHR.entry[mshr_index].da_set;
				//cout<<"set: "<< set <<endl;
				way = prath_way(set, tag0_set, tag1_set);
				//cout<<"Way: "<< way <<endl;
...
if (block[set][way].dirty) 
		...
		if (do_fill)
		{
			...
			if(MIRAGE == 1 && cache_type == IS_LLC)
			{
				...
				{
					//access_DA[set]=true;
					block[set][way].rrpv =  maxRRPV - 2;  //added by prath to assign RRPV while inserting
		...
		if(MIRAGE == 1 && cache_type == IS_LLC)
			...
			else   //Tag-miss at WB
			{
				if(SoS_ON)
				{
					...
					{
						if(sos_tag0[i].invalid_block_count <= sos_threshold) //Threshold for sending SoS should be less than 14
						{
							no_of_sos_triggered++;
							for(int k=0; k<extra_tag_ways; k++)
							{
								if(tag0[i][k].valid == 1)
								{
									set = tag0[i][k].set;
		                                			way = tag0[i][k].way;
									WQ.entry[index].ssl = set;
									WQ.entry[index].sos_way = way;
									WQ.entry[index].sos_status = true;
									break;
					...
					if(local_flag == true)
					{
						for(int i=0; i<LLC_SET; i++)
						{
							if(sos_tag1[i].invalid_block_count <= sos_threshold) //Threshold for sending SoS should be less than 14
							{
								no_of_sos_triggered++;
								for(int k=0; k<extra_tag_ways; k++)
								{
				...
				uint32_t way;
			 	if(cache_type == IS_LLC) 
			 	{
						...
						//cout<<"Handle Writeback : "<<endl;
                        			get_tag_set(WQ.entry[index].address); //sets tag0 and tag1 set number
                        			tag_number = llc_find_victim_tag(tag0_set,tag1_set,&tag_way);
                        			set = WQ.entry[index].ssl;  //prath_set();
                        			way = prath_way(set, tag0_set, tag1_set);
                        			if(tag_number ==0 )
                                			tag_set_number = tag0_set;
                        			else
				...
				else    //Tag miss
				{
					...
					{	
						...
						for(int i=0; i<LLC_SET; i++)
						{
							...
							if(sos_tag0[i].invalid_block_count <= sos_threshold) //Threshold for sending SoS should be less than 14
							{
								/*cout<<"Tag0 set"<< i << "   "<<"invalid_block_count"<<sos_tag0[i].invalid_block_count<<endl;*/
								no_of_sos_triggered++;
								...
								for(int k=0; k<extra_tag_ways; k++)
								{
									if(tag0[i][k].valid == 1)
									{
										set = tag0[i][k].set;
				                        			way = tag0[i][k].way;
										RQ.entry[index].ssl = set;
										RQ.entry[index].sos_way = way;
										RQ.entry[index].sos_status = true;
										break;
...
uint32_t CACHE::prath_set()
{		
		/*float avg_rrpv[LLC_SET] = {0.0};
		int temp_location;
		float temp_rrpv = 0.0;

		//  previous code for calculating avg_rrpv values
		for(int i = 0; i < LLC_SET; i++)
		{
			for(int j = 0; j < NUM_WAY; j++)
			{
		...
		//cout << "Set No" << set_pointer << endl;
		return temp;
		
		//cout << "Set No" << index <<  "access_DA Set" << access_DA[index] <<  endl;
		//return index;

		/*// subsequent code that uses avg_rrpv values to find temp_location
		for(int i=0; i<LLC_SET; i++)
		{
		    if(avg_rrpv[i] >= temp_rrpv)
		    {
			temp_rrpv = avg_rrpv[i];
			temp_location = i;
		    }
		}

		if(temp_location >= LLC_SET)
		{
		    assert(0);
		}

		//cout << "Set No" << temp_location << endl;
		return temp_location;*/
}


uint32_t CACHE::prath_way(uint32_t set, uint32_t tag0_set, uint32_t tag1_set)
{
	...
	while(1)
	{
		...
		{	
			if(block[set][i].rrpv == maxRRPV) //&& (block[set][i].tag_set != tag0_set || block[set][i].tag_set != tag1_set))
			{
		 ...
		 for (int i=0; i<NUM_WAY; i++)
		{
			//cout << "set" << set << "Prath_way maxRRPV" << ""  << endl;
		    block[set][i].rrpv++;
		}
	}
...
int CACHE::llc_find_victim_tag(uint32_t tag0_set,uint32_t tag1_set,int* tag_way) //returns tag_number and set tag_way
{
	...
	sos_tag0[tag0_set].invalid_block_count =  count1;
	sos_tag1[tag1_set].invalid_block_count =  count2;

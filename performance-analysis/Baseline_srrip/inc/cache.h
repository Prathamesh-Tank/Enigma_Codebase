...
struct sos_tag_array{
	int invalid_block_count = 14;  //prath
...
/*----------------------------------------CEASER-S----------------------------------------------*/
	...
	struct tag_array tag0[LLC_SET][/*extra_tag_ways */ 14], tag1[LLC_SET][/*extra_tag_ways*/ 14];
        struct sos_tag_array sos_tag0[LLC_SET], sos_tag1[LLC_SET];
...
//	     S&P---------------------------------------
	...
	//---------MIRAGE-------------
	    ...
	    uint32_t random_way(),random_set(),get_tag_set(uint64_t address),prath_way(uint32_t set, uint32_t tag0_set, uint32_t tag1_set), prath_set(); //Prathamesh

...
do
    ...
    then
	name=${TRACE%.trace.gz}
	(./../../enigma/${BINARY} -warmup_instructions ${N_WARM}000000 -simulation_instructions ${N_SIM}000000 -traces ${TRACE_DIR}/${TRACE}) > $(pwd)/../../enigma/results/${OPTION}/${name}_${N_WARM}M_${N_SIM}M.txt &
    else
	continue
    fi
done

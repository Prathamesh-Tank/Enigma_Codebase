...
/*----------------------------------------CEASER-S----------------------------------------------*/
    ...
    uint64_t total_miss_at_tag_array,no_of_sos_triggered;
    uint64_t tag_store_full_events, deferred_sos_services, deferred_sos_dirty_writebacks;
    ...
    uint64_t data_set_fill_count[LLC_SET], data_set_eviction_count[LLC_SET];
    uint64_t data_set_occupancy_histogram[LLC_WAY + 1];
    uint64_t rrpv_victim_histogram[8], rrpv_final_histogram[8];
    uint64_t total_rrpv_increment_rounds, total_rrpv_eviction_samples;
    uint64_t tag_insert_count[2], tag_occupancy_histogram[2][15];
    uint64_t sos_enqueue_events, sos_queue_max_occupancy;
    uint64_t ssl_common_path_selections, sos_forced_fill_selections;
    uint64_t stale_tag_repairs;
    //Deadblocks
	...
	uint32_t tag0_set=0,tag1_set=0;
        ...
        uint32_t sos_queue_skew[2 * LLC_SET], sos_queue_set[2 * LLC_SET];
        uint32_t sos_queue_head=0, sos_queue_tail=0, sos_queue_occupancy=0;
        uint8_t sos_pending[2][LLC_SET];
...
/*----------------------------------------CEASER----------------------------------------------*/
    ...
    Sptr=0;
    Actr=0;
    total_sets_remapped = 0;  //prath
    total_miss_at_tag_array = 0; //prath
    no_of_sos_triggered = 0;
    tag_store_full_events = 0;
    deferred_sos_services = 0;
    deferred_sos_dirty_writebacks = 0;
    total_rrpv_increment_rounds = 0;
    total_rrpv_eviction_samples = 0;
    tag_insert_count[0] = 0;
    tag_insert_count[1] = 0;
    sos_enqueue_events = 0;
    sos_queue_max_occupancy = 0;
    ssl_common_path_selections = 0;
    sos_forced_fill_selections = 0;
    stale_tag_repairs = 0;
    total_llc_stalled=0;
    total_stall_cycle=0;
...
//-------------------CEASER-s-----------------
	...
	for(int i=0;i<2048;i++)
	{
		...
		remap_Table[i] = 5000;
		marked[i]=0; //BFS
                data_set_fill_count[i] = 0;
                data_set_eviction_count[i] = 0;
                sos_pending[0][i] = 0;
                sos_pending[1][i] = 0;
	}
        for (int i = 0; i <= LLC_WAY; i++)
                data_set_occupancy_histogram[i] = 0;
        for (int i = 0; i < 8; i++) {
                rrpv_victim_histogram[i] = 0;
...
//	     S&P---------------------------------------
	...
	//---------MIRAGE-------------
	    ...
	    uint32_t random_way(),random_set(),get_tag_set(uint64_t address);
            uint32_t select_enigma_ssl_set();
            uint32_t select_enigma_common_way(uint32_t set, uint64_t ip, uint64_t full_addr, uint32_t type);
            int count_invalid_tag_entries(int tag_number, uint32_t tag_set);
            int count_valid_tag_entries(int tag_number, uint32_t tag_set);
            void enqueue_sos_candidate(int tag_number, uint32_t tag_set);
            void maybe_schedule_sos_after_tag_insert(int tag_number, uint32_t tag_set, int invalid_before_insert);
            bool service_deferred_sos_miss(PACKET *packet);
            uint32_t current_data_set_occupancy(uint32_t set);
            void sample_data_set_occupancy(uint32_t set);
            void record_data_set_fill(uint32_t set);
            void record_data_set_eviction(uint32_t set);
            void prepare_enigma_secure_miss(PACKET *packet, uint32_t tag0_set, uint32_t tag1_set);
            void resolve_enigma_fill_target(PACKET *packet, uint32_t *set, uint32_t *way);

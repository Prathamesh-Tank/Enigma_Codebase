...
// Added by prath for MIRAGE extension

uint64_t set_pointer = 0;
...
if (block[set][way].dirty) 
		...
		if (do_fill)
		{
			...
			if(cache_type == IS_LLC && block[set][way].valid == 1)
                                record_data_set_eviction(set);
			if(MIRAGE == 1 && cache_type == IS_LLC)
			{
				if(block[set][way].valid == 1)
				{
		...
		if(MIRAGE == 1 && cache_type == IS_LLC)
			...
			if(tag_number >= 0)    //gets a hit
			{
                        	...
                        	if(tag_number ==0 )
				{
					...
					//access_DA[set]=true;
					block[set][way].rrpv = 0; // added by prath for assigning rrpvv = 0 on hit
				...
				uint32_t way;
			 	if(cache_type == IS_LLC) 
			 	{
						...
						//cout<<"Handle Writeback : "<<endl;
                        			get_tag_set(WQ.entry[index].address); //sets tag0 and tag1 set number
                        			tag_number = llc_find_victim_tag(tag0_set,tag1_set,&tag_way); //To look
						resolve_enigma_fill_target(&WQ.entry[index], &set, &way);
		...
		if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
		{
			...
			if(MIRAGE == 1 && cache_type == IS_LLC)
					...
					if(tag_number ==0 )
						...
						//access_DA[set]= true;
						block[set][way].rrpv = 0;  //added by prath for Mirage update on hit
				...
				{
					prepare_enigma_secure_miss(&RQ.entry[index], tag0_set, tag1_set);
					way = -1;
				}
		...
		if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
			...
			if(MIRAGE == 1 && cache_type == IS_LLC)
					...
					if(tag_number ==0 )
                                ...
                                {
                                        prepare_enigma_secure_miss(&PQ.entry[index], tag0_set, tag1_set);
					way = -1;
                                }
                	}
...
{		
        uint64_t temp = set_pointer;
        set_pointer = (set_pointer + 1) % LLC_SET;
        return temp;
}
...
bool CACHE::service_deferred_sos_miss(PACKET *packet)
{
        ...
        for (int i = 0; i < extra_tag_ways; i++) {
                ...
                int data_way = 0;
                if (tag_number == 0) {
                        if (tag0[tag_set][i].valid != 1)
                                continue;
                        data_set = tag0[tag_set][i].set;
                        data_way = tag0[tag_set][i].way;
                } else {
                        if (tag1[tag_set][i].valid != 1)
                                continue;
                        data_set = tag1[tag_set][i].set;
                        data_way = tag1[tag_set][i].way;
                }
        ...
        remove_block_based_on_tag_array(tag_number, tag_set, selected_tag_way);
        packet->ssl = data_set;
...
int CACHE::remove_block_based_on_tag_array(int tag_number,int tag_set_number,int tag_way)
{
	...
	if(block_set >= NUM_SET || block_way < 0 || block_way >= NUM_WAY || block[block_set][block_way].valid != 1 ||
           block[block_set][block_way].tag_number != tag_number || block[block_set][block_way].tag_set != tag_set_number ||
...
//Load Aware ends
	...
	else if(tag == 1)
	{
        ...
        tag_store_full_events++;
        *tag_way = rand() % extra_tag_ways;
        return tag;
...
//	cout<<"Fill Tag of tag_number :" <<tag_number<<" tag_set :"<<tag_set<<" tag_way : " <<tag_way<<" tag : "<<tag<<endl;
	...
	/*for(int i=0;i<NUM_SET;i++)
        ...
        int invalid_before_insert = count_invalid_tag_entries(tag_number, tag_set);
	if(tag_number == 0 )
	{
                tag_insert_count[0]++;
		...
		block[block_set][block_way].tag_way = tag_way;
                maybe_schedule_sos_after_tag_insert(tag_number, tag_set, invalid_before_insert);
		return;
	}
	else if(tag_number == 1 )
	{

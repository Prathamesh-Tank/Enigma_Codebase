		...
		if(MAYA == 1 && cache_type == IS_LLC)
		{
			...
			if(tag_number == 0)
				...
				tag_set_number = tag1_set;
                        ...
                        if (hit) {
			        prepare_enigma_secure_miss(&MSHR.entry[mshr_index], tag0_set, tag1_set);
			        resolve_enigma_fill_target(&MSHR.entry[mshr_index], &set, &way);
                        } else {
                                // First touch remains tag-only in Maya semantics.
                                set = 0;
                                way = 0;
                        }
				...
				if (hit) {
					...
					if (tag_number == 0) {
						...
						tag1[tag_set_number][tag_way].priority = 1;
                                        ...
                                        maya_promotion_hits_to_data_store++;
                                        record_data_set_fill(set);
                                        llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0);
					// Maya tag-only hits are promoted into the LLC data store here.
					num_pr0--;
				}
				else {
					// Original Maya first touch is tag-only. In this variant we still pick the
					// data-store target only when the line is later promoted on a tag hit.
					...
					if (MSHR.entry[mshr_index].address == 1084953){
						cout << "FILL" << endl;
                                        ...
                                        // Tag placement still follows the same two-skew, load-aware choice.
                                        int invalid_before_insert = count_invalid_tag_entries(tag_number, tag_set_number);
					fill_tag(tag_number,tag_set_number,tag_way,MSHR.entry[mshr_index].address,set,way,false);	
                                        maybe_schedule_sos_after_tag_insert(tag_number, tag_set_number, invalid_before_insert);
					if (tag_number == 0) {
						assert(tag0[tag_set_number][tag_way].valid == 1);
						assert(MSHR.entry[mshr_index].address == tag0[tag_set_number][tag_way].tag);
                                                assert(tag0[tag_set_number][tag_way].priority == 0);
					}
					else if (tag_number == 1) {
						assert(tag1[tag_set_number][tag_way].valid == 1);
						assert(MSHR.entry[mshr_index].address == tag1[tag_set_number][tag_way].tag);
                                                assert(tag1[tag_set_number][tag_way].priority == 0);
					}
                                        maya_tag_only_first_touches++;
                                        num_pr0++;
				}
		...
		if (way >= 0) { // writeback hit (or RFO hit for L1D)
			...
			if (cache_type == IS_L1D) { // RFO miss
					...
					if(MAYA == 1 && cache_type == IS_LLC)
					{
						...
						if(tag_number == 0)
							...
							assert(0);
                                                ...
                                                if (tag_entry_type == -2) {
                                                        prepare_enigma_secure_miss(&WQ.entry[index], tag0_set, tag1_set);
                                                        uint32_t promoted_way = 0;
                                                        resolve_enigma_fill_target(&WQ.entry[index], &set, &promoted_way);
                                                        way = (int)promoted_way;
                                                } else {
                                                        // First-touch writebacks also remain tag-only.
                                                        set = 0;
                                                        way = 0;
                                                }
				...
				if (block[set][way].dirty) {
					...
					if (MAYA == 1 && cache_type == IS_LLC && tag_entry_type == -1)
					{
						if(num_pr0 >= threshold_pr0) {
							...
							if (tag_number == 0) {
								...
								if (tag1[tag_set_number][tag_way].priority == 1) {
									...
									num_pr0--;
                                                ...
                                                int invalid_before_insert = count_invalid_tag_entries(tag_number, tag_set_number);
						fill_tag(tag_number,tag_set_number,tag_way,WQ.entry[index].address,set,way,false);
                                                maybe_schedule_sos_after_tag_insert(tag_number, tag_set_number, invalid_before_insert);
						if(tag_number == 0) {
							assert(tag0[tag_set_number][tag_way].valid == 1);
                                                        assert(tag0[tag_set_number][tag_way].priority == 0);
						...
						if ((WQ.entry[index].fill_level < fill_level) && upper_level_dcache[writeback_cpu])
							upper_level_dcache[writeback_cpu]->return_data(&WQ.entry[index]);
						WQ.remove_queue(&WQ.entry[index]);
						if (WQ.occupancy)
							WQ.next_schedule_index = WQ.head;
						return;
					}
					if (do_fill) 
					{
...
uint32_t CACHE::select_enigma_ssl_set()
{
        // SSL is a fill-placement policy only, so hits do not advance this pointer.
        uint32_t selected = ssl_pointer;
        ssl_pointer++;
        if (ssl_pointer >= NUM_SET)
                ssl_pointer = 0;
        return selected;
}

uint32_t CACHE::select_enigma_common_way(uint32_t set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
        return llc_find_victim(cpu, 0, set, block[set], ip, full_addr, type);
}

int CACHE::count_invalid_tag_entries(int tag_number, uint32_t tag_set)
{
        int invalid_count = 0;
        for (int way = 0; way < extra_tag_ways; way++) {
                if (tag_number == 0) {
                        if (tag0[tag_set][way].valid == 0)
                                invalid_count++;
                } else {
                        if (tag1[tag_set][way].valid == 0)
                                invalid_count++;
                }
        }
...
int CACHE::count_valid_tag_entries(int tag_number, uint32_t tag_set)
{
        return extra_tag_ways - count_invalid_tag_entries(tag_number, tag_set);
}

void CACHE::enqueue_sos_candidate(int tag_number, uint32_t tag_set)
{
        if (!ENIGMA_ENABLE_SOS_SUPERVISOR)
                return;
        if (sos_pending[tag_number][tag_set] == 1)
                return;
        if (sos_queue_occupancy >= (2 * LLC_SET))
                return;

        sos_pending[tag_number][tag_set] = 1;
        sos_queue_skew[sos_queue_tail] = tag_number;
        sos_queue_set[sos_queue_tail] = tag_set;
        sos_queue_tail = (sos_queue_tail + 1) % (2 * LLC_SET);
        sos_queue_occupancy++;
        sos_enqueue_events++;
        if (sos_queue_occupancy > sos_queue_max_occupancy)
                sos_queue_max_occupancy = sos_queue_occupancy;
}

void CACHE::maybe_schedule_sos_after_tag_insert(int tag_number, uint32_t tag_set, int invalid_before_insert)
{
        if (!ENIGMA_ENABLE_SOS_SUPERVISOR)
                return;

        // Detection and service are split: record the vulnerable tag set now,
        // but defer the forced eviction to a later miss.
        int invalid_after_insert = count_invalid_tag_entries(tag_number, tag_set);
        if (invalid_before_insert > ENIGMA_SOS_THRESHOLD && invalid_after_insert <= ENIGMA_SOS_THRESHOLD)
                enqueue_sos_candidate(tag_number, tag_set);
}

uint32_t CACHE::current_data_set_occupancy(uint32_t set)
{
...
void CACHE::sample_data_set_occupancy(uint32_t set)
{
        uint32_t occupancy = current_data_set_occupancy(set);
        if (occupancy <= LLC_WAY)
                data_set_occupancy_histogram[occupancy]++;
}

void CACHE::record_data_set_fill(uint32_t set)
{
        data_set_fill_count[set]++;
        sample_data_set_occupancy(set);
}

void CACHE::record_data_set_eviction(uint32_t set)
{
        data_set_eviction_count[set]++;
        sample_data_set_occupancy(set);
}

bool CACHE::service_deferred_sos_miss(PACKET *packet)
{
        if (!ENIGMA_ENABLE_SOS_SUPERVISOR || sos_queue_occupancy == 0)
                return false;

        uint32_t tag_number = sos_queue_skew[sos_queue_head];
        uint32_t tag_set = sos_queue_set[sos_queue_head];
        sos_queue_head = (sos_queue_head + 1) % (2 * LLC_SET);
        sos_queue_occupancy--;
        sos_pending[tag_number][tag_set] = 0;

        int valid_list[15];
        int valid_count = 0;
        for (int i = 0; i < extra_tag_ways; i++) {
                if (tag_number == 0) {
                        if (tag0[tag_set][i].valid != 1 || tag0[tag_set][i].priority != 1)
                                continue;
                } else {
                        if (tag1[tag_set][i].valid != 1 || tag1[tag_set][i].priority != 1)
                                continue;
                }

                uint32_t data_set = (tag_number == 0) ? tag0[tag_set][i].set : tag1[tag_set][i].set;
                int data_way = (tag_number == 0) ? tag0[tag_set][i].way : tag1[tag_set][i].way;
                if (data_set >= NUM_SET || data_way < 0 || data_way >= NUM_WAY)
                        continue;
                if (block[data_set][data_way].valid != 1)
                        continue;
                if (block[data_set][data_way].tag_number != tag_number || block[data_set][data_way].tag_set != (int)tag_set || block[data_set][data_way].tag_way != i)
                        continue;
        ...
        // SoS ignores RRPV and randomly evicts from the recorded vulnerable tag set.
        int selected_tag_way = valid_list[rand() % valid_count];
        uint32_t data_set = (tag_number == 0) ? tag0[tag_set][selected_tag_way].set : tag1[tag_set][selected_tag_way].set;
        int data_way = (tag_number == 0) ? tag0[tag_set][selected_tag_way].way : tag1[tag_set][selected_tag_way].way;
        if (block[data_set][data_way].valid == 1 && block[data_set][data_way].dirty == 1)
                deferred_sos_dirty_writebacks++;
        if (!remove_block_based_on_tag_array(tag_number, tag_set, selected_tag_way))
                return false;
        packet->ssl = data_set;
        packet->sos_way = data_way;
        packet->sos_status = true;
        deferred_sos_services++;
        no_of_sos_triggered++;
        return true;
}

void CACHE::prepare_enigma_secure_miss(PACKET *packet, uint32_t tag0_set, uint32_t tag1_set)
{
        packet->sos_status = false;
        packet->sos_way = 0;
        total_miss_at_tag_array++;
        if (service_deferred_sos_miss(packet)) {
                sos_forced_fill_selections++;
                return;
        }

        ssl_common_path_selections++;
        packet->ssl = select_enigma_ssl_set();
}

void CACHE::resolve_enigma_fill_target(PACKET *packet, uint32_t *set, uint32_t *way)
{
        if (packet->sos_status) {
                // Deferred SoS reuses the data-store slot of the forcibly evicted block.
                *set = packet->ssl;
                *way = packet->sos_way;
                return;
        }

        if (ENIGMA_USE_GLOBAL_RANDOM_DATA) {
                *set = random_set();
                *way = random_way();
                return;
        }

        // Common-path fills use round-robin SSL for set choice and local SRRIP within that set.
        *set = packet->ssl;
        cpu = packet->cpu;
        *way = select_enigma_common_way(*set, packet->ip, packet->full_addr, packet->type);
        cpu = 0;
}
...
int CACHE::remove_block_based_on_tag_array(int tag_number,int tag_set_number,int tag_way)
{
	...
	*/
	...
	if (tag_valid != 1)
                return 0;
        if (block_set >= NUM_SET || block_way < 0 || block_way >= NUM_WAY || block[block_set][block_way].valid != 1 ||
            block[block_set][block_way].tag_set != tag_set_number || block[block_set][block_way].tag_way != tag_way) {
                if (tag_number == 0) {
                        tag0[tag_set_number][tag_way].valid = 0;
                        tag0[tag_set_number][tag_way].tag = 0;
                        tag0[tag_set_number][tag_way].set = 0;
	...
	cout << "SAE" << sae_count << endl;
        record_data_set_eviction(block_set);

...
rm -f inc/cache_mode.h
touch inc/cache_mode.h
echo -e "#ifndef CACHE_MODE_H" >> inc/cache_mode.h
echo -e "#define CACHE_MODE_H" >> inc/cache_mode.h
echo -e "#define MIRAGE 1" >> inc/cache_mode.h
echo -e "#define ENIGMA_ENABLE_SSL_SRRIP 1" >> inc/cache_mode.h
echo -e "#define ENIGMA_ENABLE_SOS_SUPERVISOR 1" >> inc/cache_mode.h
echo -e "#define ENIGMA_USE_GLOBAL_RANDOM_DATA 0" >> inc/cache_mode.h
echo -e "#define LLC_SECURITY_MODE \"enigma+maya\"" >> inc/cache_mode.h
...
echo "Remapping : ${remapping}"
 if [ "$NUM_CORE" -gt "1" ]; then
	BINARY_NAME="${BRANCH}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${CEASER_S_LLC}-${partitions}-${pipelined_encryption_engine}-${CEASER_LATENCY}-${remapping}-${remapping_on_eviction}-${NUM_CORE}core_enigma+maya"
else
        BINARY_NAME="${BRANCH}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${CEASER_S_LLC}-${partitions}-${pipelined_encryption_engine}-${CEASER_LATENCY}-${remapping}-${remapping_on_eviction}-${NUM_CORE}core-${LLC_SLICES}core_enigma+maya"
 fi
